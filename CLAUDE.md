...
| `styles/a11y.css` | Una sola clase, `.sr-only`: texto que existe para el lector de pantalla y no se dibuja. Es global porque la usan tres componentes de dos capas distintas, y porque `display: none` o `visibility: hidden` —lo que saldría por instinto— sacan el elemento del árbol de accesibilidad y no valen. Va anclada en `top: 0; left: 0` y no es opcional: con `position: absolute` y sin ancestro posicionado, su bloque contenedor es el viewport, así que en su posición estática se escapa del scroll de `main` y estira el documento —un badge con `label` al final de la styleguide añadía una segunda barra de scroll—. Anclada en la esquina no mide nada, y al lector de pantalla la posición le da igual. |
