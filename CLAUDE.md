...
Cada uno en su carpeta, con `.ts` + `.html` + `.css`, exportado desde el barrel `src/app/shared/ui/index.ts`. `field-shell/control-state.ts` es la excepción: no es un componente sino lo que los cinco controles comparten —el contador de identificadores, el primer error, el placeholder visible y el id del mensaje—, que estaba escrito cuatro veces.
