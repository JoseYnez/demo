...
| `windowApi` | `window\|minimize`, `window\|toggle_maximize`, `window\|close`, `window\|is_maximized`, `window\|set_theme`, `window\|set_fullscreen`, `window\|is_fullscreen`, `window\|maximize`, `window\|unmaximize`, y el listener `onWindowChanged` | Barra de título propia (§3.7). Expone `enTauri`; fuera de Tauri —`pnpm start`, tests— los métodos son no-op **salvo los tres de pantalla completa**, que caen a la Fullscreen API del navegador (§3.7). El resto es no-op para que el shell y el `ThemeService` funcionen igual en el navegador. |

**Nada emite eventos desde Rust todavía**, pero ya hay un listener: el `onWindowChanged` de `windowApi`, que mantiene el icono de maximizar/restaurar/salir en su sitio. Se llama así y no `onResized` porque el evento no es el mismo en los dos sitios —`onResized` de Tauri en la ventana, `fullscreenchange` del DOM en el navegador— y quien lo consume no debe tener que saber cuál le toca. Su `unlisten` va registrado en `destroyRef.onDestroy` en [app.ts](src/app/app.ts) — el patrón de §6.6, y el que hay que copiar para el siguiente. Los permisos de eventos ya están concedidos (`core:default` incluye `core:event:default` → `allow-listen`, `allow-unlisten`, `allow-emit`, `allow-emit-to`), así que añadir listeners no requiere tocar `capabilities/`.
...
Los ocho de ventana son los que **no** trae `core:window:default`. Lo que sí trae y por eso no aparece aquí: `is-maximized` (el icono de restaurar), **`is-fullscreen`** (con el que `FullscreenService` se resincroniza en cada `onWindowChanged`), `theme` e `internal-toggle-maximize` (el doble clic en la barra para maximizar).
...
- **Con la misma combinación gana el último registrado**, y al darse de baja el control vuelve al anterior. Nada se sobrescribe: los dos conviven en la pila, sólo cambia cuál responde. Es el mecanismo para que un diálogo se quede `Esc` mientras está abierto y lo devuelva al cerrarse.
- **`register()` devuelve su propia baja**, además de la automática contra el `DestroyRef`. Es lo que permite registrar un atajo que va y viene sin montar y desmontar un componente: hoy lo usa el Esc de la pantalla completa (§3.7), desde un `effect()` que lo da de alta al entrar y de baja al salir. La baja es idempotente y retira también el `onDestroy`, para que alternar mil veces no acumule mil callbacks muertos.
...
Registrado hoy en el shell: **Ctrl/Cmd + Alt + T** (tema claro/oscuro), **F11** (pantalla completa, §3.7) y **Esc** (salir de pantalla completa), este último sólo mientras la pantalla completa está activa. Los de la styleguide se registran en su propio componente y desaparecen al salir de la página — el banco de pruebas de `/styleguide` enseña justamente eso.
