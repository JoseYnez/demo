...
`app.windows[0].decorations` vale **`false`**: la barra de la app *es* la barra de la ventana. Icono de 18 px y navegación a la izquierda; a la derecha tema, notificaciones y los tres botones de ventana, todo en **36 px**. Estuvo en 44 y era demasiado: 8 px de aire que ninguna de las tres zonas usaba, en una franja que se lleva alto de todas las pantallas y no lo devuelve. La referencia es la barra de VS Code (35 px), que carga navegación igual que ésta; la nativa de Windows 11 son 32, y por debajo de 36 los enlaces —27,5 px de contenido— dejan de tener margen. Los botones de acción bajaron a 28 px con ellos: a 30 el hover casi tocaba los dos bordes y la barra parecía llena de botones. Dos barras apiladas —una nativa que no se puede tematizar y otra nuestra justo debajo— era el precio de no tomar esta decisión.

Lo que hay que saber para no romperlo:

- **La zona de arrastre es `data-tauri-drag-region="deep"` en la barra**, no en cada hueco. El script de Tauri camina el `composedPath` hacia arriba y **corta en el primer elemento clicable** (`BUTTON`, `A`, `INPUT`, `[tabindex]`, roles interactivos), así que los enlaces y los botones siguen recibiendo su click sin declarar nada. El único opt-out (`data-tauri-drag-region="false"`) lo llevan las dos capas que cuelgan de la campana (§3.10): son lo único de la barra con fondo propio y con texto que uno quiere copiar, y sin él `mousedown` arrastraría la ventana desde el hueco entre dos filas y el texto dejaría de poderse seleccionar.
...
- **`NotificationsService` (`core/services/`) es la única fuente del contador**, con el patrón signal privado + `readonly` y un `computed` de las no leídas. Pulsar la campana **abre el panel** (§3.10), que es de donde cuelgan también los avisos flotantes; el banco de pruebas de `/styleguide#notificaciones` sigue siendo la única forma de provocarlos.
...
- **`sync()` se calla mientras hay una transición en vuelo, y lo comprueba dos veces.** Desmaximizar y entrar en pantalla completa son dos cambios de tamaño, y el `onWindowChanged` del primero llega con `isFullscreen()` todavía en `false`: sin la guarda, la barra reaparecía a mitad del gesto. La segunda comprobación va **después** del `await`, porque la primera sólo descarta los `sync` que *empiezan* durante la transición: uno que ya estaba esperando la respuesta de la ventana cuando el usuario pulsó F11 la contestaría con el estado viejo y pisaría el cambio. Lo enuncia el test «descarta un sync que ya estaba en vuelo cuando arranca la transición».
- **En pantalla completa la barra se queda, pero el botón de tamaño cambia de oficio.** Deja de maximizar y pasa a **salir de pantalla completa**, con su propio icono —cuatro esquinas apuntando adentro— y su `aria-label`. Es la única retroalimentación de que estás en pantalla completa y la única salida con ratón; sin ella el botón seguía dibujando «maximizar» sobre una ventana que ya ocupaba el monitor, que es mentira. `FullscreenService.active` existía desde el principio y **no lo leía nadie**: éste es su consumidor.
- **Se sale con F11, con Esc o con ese botón.** Esc se registra **sólo mientras la pantalla completa está activa** y se da de baja al salir, con el mecanismo de pila de §11.6 — el mismo que usará el primer diálogo. Registrarlo siempre con un `if` dentro habría dejado un Esc muerto en `list()`, o sea en el panel de ayuda, mintiendo el 99% del tiempo.
...
### 3.10 Notificaciones: el panel y el aviso

Un aviso tiene **dos caras y una sola fuente**: el **toast** es la pasajera y el **panel** la permanente. Las dos cuelgan de la campana, y ninguno de los dos componentes conoce el servicio — el shell les pasa la lista y recoge sus eventos, que es lo que permite montarlos con datos de mentira en `/styleguide#notificaciones`.

| Pieza | Dónde | Qué hace |
|---|---|---|
| `AppNotification` | `models/notification.model.ts` | `variant`, `title`, `detail?`, `createdAt`, `duration`, `read`. `NewNotification` es lo que se pasa a `push()` y añade `silent`. |
| `NotificationsService` | `core/services/notifications.ts` | `items` (el historial), `unread` (el contador) y `toasts` (la cola visible, derivada de las dos). `push()` da de alta y encola; `expireToast` / `closeToast` / `clearToasts` gobiernan la cola; `dismiss` / `clear`, el historial. |
| `Toast` | `shared/ui/toast/` | La cara pasajera (§11.3). |
| `NotificationPanel` | `shared/ui/notification-panel/` | La cara permanente (§11.3). |
| `notificationApi` | `tauri/notification.api.ts` | El wrapper de §7 hacia el comando `notify`, para cuando el aviso tiene que salir de la ventana. |
| `NotificationService` (Rust) | `src-tauri/src/services/notification.rs` | Registra el AppUserModelID, deja el icono en disco y manda el toast del SO. |
| `NotificationTitle` / `NotificationBody` | `src-tauri/src/models/notification.rs` | Los newtypes del límite: título obligatorio, y lo largo se recorta en vez de perderse. |
| El shell | [app.ts](src/app/app.ts) / [app.html](src/app/app.html) | Abre y cierra el panel, ancla las dos capas a la campana y decide qué se da por leído. |

Decisiones que no son negociables sin leer esto:

- **Abrir el panel no marca nada; cerrarlo sí.** Marcarlas al abrir borra el énfasis justo en el instante en que sirve: el punto relleno y el título en negrita están para poder ver qué es nuevo *mientras* lo miras. Y al cerrar ya lo has visto, así que dejar el contador encendido sería mentira. La campana lo marcaba todo al pulsarla sin enseñar nada — un contador que se apagaba sin que nadie hubiera leído.
- **Cerrar un toast con la aspa lo da por leído; que expire, no.** Pulsar la × es prueba de que estabas delante. Que se cumpla el tiempo no prueba nada —pudo pasar con la ventana de fondo—, así que el aviso sigue contando en la campana.
- **El error no se va solo.** `danger` nace con `duration: 0`; el resto, con 6 s. Un aviso que desaparece antes de leerse no avisó de nada, y es la única familia donde eso importa lo suficiente para dejar basura en pantalla. Una `duration` explícita gana a la de la familia.
- **Tres toasts a la vez como mucho, los más nuevos arriba.** Diez avisos de golpe son diez cosas que nadie lee; lo que se cae de la pila no se pierde, está en el panel.
- **El historial se queda con los cincuenta últimos.** Un panel que crece sin fin es una fuga con interfaz. Y no es un log: eso es la decisión de §15.
- **`silent` mete el aviso en el historial sin asomarlo.** Para lo que conviene poder consultar y no interrumpe.
- **Mientras el panel está abierto no asoma ningún toast, y abrirlo vacía la cola.** Los dos cuelgan del mismo punto: un toast encima del panel taparía justo la fila que acaba de nacer.
- **Las dos capas se anclan a la campana con `position: absolute`, no al borde de la ventana.** Dentro de Tauri la campana no está pegada a la derecha —los tres botones de ventana van detrás—, así que alinear con el borde de la pantalla las habría dejado flotando lejos de lo que las abrió.
...
- **El título es obligatorio; lo largo se recorta, no se rechaza.** 120 caracteres el título y 400 el cuerpo, cortando por carácter y no por byte. Perder un aviso por pasarse de largo sería peor que enseñarlo recortado; lo que sí es un error de quien llama —y por eso sí se rechaza— es un aviso sin título.
- **Aun así no es un canal fiable, que es justo por lo que no es el único.** El Asistente de concentración y el interruptor por app de Windows la silencian sin decir nada. Nada de la app puede depender de que salga.
- **Si el envío falla, lo recoge el `ErrorHandler` global (§8.2).** El servicio lanza la promesa con `void` y no la captura: tragárselo dejaría un fallo del SO invisible, y enseñárselo al usuario sería un aviso sobre un aviso.
- **`push()` sigue siendo la única puerta.** Quien avisa no elige el canal —no puede saber si la ventana tiene el foco—; lo elige el servicio. Por eso `silent` sigue significando lo mismo: ni toast, ni sistema, sólo historial.
...
│   ├── styles/                 # fonts, tokens, reset, forms, buttons, panels + testing/
...
**Controles propios**: implementar `FormValueControl<T>` (o `FormCheckboxControl` para booleanos). El único requisito es un `model()` llamado `value`; el resto son opcionales que Angular **sincroniza solo** si los declaras — `errors`, `touched`, `dirty`, `disabled`, `readonly`, `required`, `invalid`, `name`, `minLength`, `maxLength`, `pattern`, y el output `touch`. Los seis controles de la base declaran `errors`, `disabled`, `required`, `touched`, `name` y `touch`; `readonly` lo llevan `Input`, `Textarea`, `Combobox`, `Select` y `DateRangePicker`, y los dos últimos lo mapean a deshabilitado más `aria-readonly` porque su elemento nativo no lo admite.
...
| `styles/panels.css` | Base compartida del control que abre un panel anclado: `.ui-anchor`, `.ui-trigger`, `.ui-panel` y su rejilla de presets y campos. Está aquí por lo mismo que las dos de arriba: la comparten `DateRangePicker`, `DateTimeRangePicker` y `Combobox`, y de las 168 líneas que tenía el primero, 166 eran de los dos. Cada componente escribe sólo lo suyo —el ancho del panel, si los campos van en una o dos columnas— y gana en la cascada por el atributo de encapsulación. |
...
- **Altura de los controles**: `--control-h`, `--control-h-float` y `--control-h-inset` (34/40/52 px). Están en la capa global y no en cada componente porque los tres modos de etiqueta valen para todos los controles.
...
**El trazo va pegado al control; el hueco es cosa de los botones.** Un control con borde ya dibuja su contorno, así que un anillo separado 2 px le pinta un segundo rectángulo alrededor: el campo parecía crecer al enfocarse, y en modo `float` competían tres líneas —el borde con muesca, el hueco y el anillo—. Los controles llevan ahora el trazo **encima de su propio borde** (`outline-offset: -1px`: 2 px de acento a ras), y el `float` no lleva outline en absoluto, sino que engorda su borde con muesca a 2 px — un outline o un `box-shadow` cruzarían el hueco del `<legend>` y taparían la etiqueta, porque **sólo el borde sabe dónde está la muesca**. A cambio la etiqueta flotada se corre a `--space-2 + 2px` mientras dura el foco, o asoma el pellizco de borde que ya avisa §11.3. Los botones **sí** conservan `outline-offset: 2px`: en un relleno el trazo por dentro sería invisible en tema claro, donde `--accent-fg` es el mismo color que `--accent`, y darle al `secondary` un tratamiento propio dejaría dos geometrías de foco en la misma fila de botones. El halo pasó de 3 a 4 px porque debajo del trazo no se veía: sobre blanco, los 2 px del outline se comían casi todo.
...
Cada uno en su carpeta, con `.ts` + `.html` + `.css`, exportado desde el barrel `src/app/shared/ui/index.ts`. `field-shell/control-state.ts` es la excepción: no es un componente sino lo que los seis controles comparten —el contador de identificadores, el primer error, el placeholder visible y el id del mensaje—, que estaba escrito cuatro veces. `anchored-panel/anchored-panel.ts` es la segunda por lo mismo: la clase que gobierna un panel colgado de un disparador —abrir y cerrar, colocar, el Esc, el cierre por luz y la vuelta del foco—, que usan los dos pickers y el `Combobox`. Su único parámetro de comportamiento es `enfocarElPanel`, que el combobox pone a `false` porque en un combobox el foco no entra al panel (ver abajo); todo lo demás vale igual para los tres. No es un `<app-popover>` con proyección porque eso obligaría a meter el disparador dentro y cambiaría el DOM de todos sus consumidores; se construye en un inicializador de campo o en el constructor, porque necesita contexto de inyección para sus `effect()`.
...
| `Combobox` | `<app-combobox>` | `label`, `labelMode`, `options` (`ComboboxOption[]`, requerido), `placeholder`, `hint`, `emptyMessage`, `autocomplete`, `clearOnOpen` + el contrato de §6.8 | `FormValueControl<string>`. Expone `focus()` y emite `picked`. Campo de texto **libre** con sugerencias propias: el valor es lo escrito, esté o no en la lista. Ver abajo. |
...
| `DateRangePicker` | `<app-date-range-picker>` | `label`, `labelMode`, `placeholder`, `presets` (`DateRangePreset[]`), `minDate`, `maxDate`, `hint` + contrato | `FormValueControl<DateRange \| null>`. Expone `focus()`. Un botón que abre un panel con los rangos habituales y las dos fechas del rango escrito. Ver abajo. |
| `DateTimeRangePicker` | `<app-date-time-range-picker>` | `label`, `labelMode`, `placeholder`, `presets` (`DateTimeRangePreset[]`), `minDateTime`, `maxDateTime`, `minuteStep`, `hint` + contrato | `FormValueControl<DateTimeRange \| null>`. Expone `focus()`. El hermano con reloj del anterior: el valor es un **instante en UTC** y lo que se ve es la hora del equipo (§11.9). Ver abajo. |
...
| `FieldShell` | `<app-field-shell>` | `labelMode`, `label`, `controlId`, `floated`, `required`, `disabled`, `hint`, `error` | Carcasa que comparten todos los controles: etiqueta, muesca y línea de ayuda/error. Sólo se usa directamente al construir un control propio. |
...
- Lo rechazado —tipo, tamaño, cupo, duplicado, carpeta— **no entra al modelo**:
  ...
  **error del formulario > aviso de rechazo > hint**. El aviso además se dobla
  ...
  lea. Se anuncia solo porque responde a una acción en vez de aparecer al
  perder el foco; el aviso de *Aplicar* del `DateRangePicker` y el rechazo del
  acceso (§3.9) son los otros dos, y por lo mismo.
...
**Toast — el color va al filo, no al fondo.** El aviso flota, así que se pinta como todo lo que flota en el sistema: `--bg-surface-raised` + `--shadow-lg` + `--edge-raised` (§11.2). Rellenarlo con el fondo tenue de su familia era lo primero que se probó y **no funciona en oscuro**: `--color-danger-bg` está en L .29 y la superficie elevada en L .31, así que el aviso se hundía en la página en vez de despegarse. La familia se lleva al **filo de 3 px de la izquierda y al icono**, en `--color-*-fg`, que sirve de trazo en los dos temas. `neutral` usa `--border-strong`.

- **La cuenta atrás se ve, y se detiene a la vista.** Una barra de 2 px al pie —el sitio que ya ocupa el progreso del `GestureButton`, y por lo mismo: encima del texto no queda margen de contraste— se vacía en `duration`. Es una animación CSS que se pausa con `:hover` y `:focus-within`, y el temporizador de JS se para con los mismos dos hechos, así que lo que se ve y lo que se cuenta no pueden separarse. Con `prefers-reduced-motion` la barra no se dibuja; el temporizador sigue igual.
- **La animación se declara con longhands (`animation-name`, `animation-duration`, …), no con el atajo.** `animation-duration: var(--toast-ms)` funciona; meter la variable en el shorthand es la trampa de §6.9.
- **Pausa el puntero encima y también el foco dentro**, y hacen falta las dos: un usuario de teclado que llega a la aspa nunca genera `hover`, y WCAG 2.2.1 pide poder alargar lo que se mueve solo. Se guardan como dos hechos separados, no como un contador de pausas, porque `focusin` puede repetirse sin su `focusout` y un contador desbalanceado deja el aviso congelado para siempre.
- **El componente no sabe qué es una notificación**: recibe texto y una duración, y avisa de que se acabó. Quién lo encoló y qué significa cerrarlo es del shell (§3.10).

**NotificationPanel — el punto relleno es "sin leer".** Cada fila lleva a la izquierda un punto de 8 px en el color de su familia: **relleno mientras esté sin leer, sólo trazo cuando ya se vio**. Es la misma distinción entre relleno y trazo que separa las dos apariencias del `Badge`, aplicada a la marca más pequeña del sistema, y evita inventar un indicador nuevo para un estado que ya tenía vocabulario. La refuerzan el título, que pasa de `--text-primary` en medium a `--text-secondary` normal, y nada más: teñir la fila entera de `--accent-subtle` convierte diez avisos sin leer en un muro de color.
...
**Combobox — la lista ayuda, no manda, y el desplegable es propio.** Es el hueco que deja el `Select`, que sólo admite lo que ofrece: aquí el valor es lo que hay escrito, esté o no entre las sugerencias, y quien necesite restringirlo pone un validador en su formulario. De ahí se sigue todo lo demás:

- **`ComboboxOption` no tiene `value`, sólo `label` y un `detail` opcional.** Con texto libre, lo que se elige *es* lo que se escribe en el campo: un `value` distinto del `label` sólo podría mostrarse o guardarse, nunca las dos cosas. `detail` es la línea secundaria de la fila —la comunidad de una provincia, el país de una ciudad— y **entra en la búsqueda**, que es la mitad de la razón para no usar el nativo. Como el `label` es el valor, dos opciones con el mismo `label` son la misma opción: por eso el `@for` lo usa de `track`.
- **El desplegable no es un `<datalist>`.** Es el mismo listón con el que §11.2 descartó seguir peleando con el `<select>` nativo: el popup del motor no admite ni un token —ni la tipografía, ni el detalle en dos líneas, ni el resaltado—, y el criterio de filtrado (subcadena o prefijo) cambia según el motor, que es exactamente la divergencia que esta base evita. El panel cuelga del `AnchoredPanel` que ya existía, así que se voltea hacia arriba y se alinea a la derecha sin código nuevo.
- **El foco no entra nunca a la lista**, y por eso `AnchoredPanel` estrena `enfocarElPanel: false`. Es el patrón de combobox de ARIA: el foco se queda en el campo —si no, no se podría seguir escribiendo— y la opción activa viaja en `aria-activedescendant`. El `pointerdown` de cada fila hace `preventDefault()` por lo mismo: sin él, pulsar una opción sacaría el foco del campo y el `focusout` cerraría el panel antes de que llegara el `click`.
- **Filtra por subcadena, sin tildes y sin mayúsculas.** «rid» encuentra Madrid y «avila» encuentra Ávila; la eñe se pliega a ene, como en cualquier buscador. El resaltado se pinta sobre el texto **original** —con su tilde—, y eso obliga a un mapa de índices: plegar puede cambiar la longitud de la cadena (una tilde suelta desaparece), así que cortar por la posición de la coincidencia plegada partiría la palabra por donde no es. Lo enuncia el test «no descoloca el resaltado cuando plegar cambia la longitud» de [suggestions.spec.ts](src/app/shared/ui/combobox/suggestions.spec.ts).
- **Se resalta con el peso, no con el acento.** Un `--accent-fg` sobre el `--overlay-hover` de la fila es justo el par que §15 tiene medido por debajo de AA en tema claro; el semibold no arrastra ningún requisito de contraste y se ve igual en los dos temas.
- **Filtra en memoria y no llama al backend**, por lo mismo que la búsqueda de contactos (§3.11): con una lista que cabe en el panel, un comando sería latencia a cambio de nada. No hay perilla para desactivarlo — dos perillas que pueden contradecirse son un bug esperando, y el día que las sugerencias lleguen ya filtradas de un servidor, eso es una decisión que se toma entonces.
- **Escribir no destaca nada; destacar es un gesto explícito con las flechas.** Auto-destacar la primera coincidencia es lo que hacen los comboboxes cerrados, y en uno de texto libre significa que `Enter` sustituye en silencio lo que acabas de teclear. Sin nada destacado, `Enter` no se previene y el formulario se envía con lo escrito.
- **La lista no se despliega para decir que no hay nada.** Si ya estaba abierta se queda, con el mensaje de vacío —que es información útil a media búsqueda—; si estaba cerrada, no asoma. Al revés, un campo mayormente libre saltaría un cartel de «Sin coincidencias» en cada tecla, tapando lo que hay debajo.
- **Pulsar el campo despliega la lista entera; volver a pulsarlo no la cierra.** Abrir olvida el filtro, así que el desplegable siempre enseña todo lo que hay —tras elegir «Madrid», reabrir no deja sólo Madrid—. Y cerrar con el mismo click convertiría en trampa el gesto de recolocar el cursor: se cierra con `Esc`, con `Tab`, pulsando fuera o eligiendo.
- **`clearOnOpen` vacía el texto, nunca el modelo.** Es opcional y viene apagado. Abrir la lista vacía el campo —para no tener que seleccionar y borrar antes de buscar otra cosa— y cerrarlo sin haber tocado nada devuelve el valor; lo que se escriba se queda, salga o no de la lista, porque el campo sigue siendo de texto libre. **El modelo no se toca hasta que se teclea o se elige**, y eso no es un detalle: vaciarlo marcaría el formulario como `dirty` y dispararía el aviso de cambios sin guardar (§11.8) sobre un campo que nadie editó, y en uno `required` ya tocado asomaría el error mientras se escribe. Con el modelo intacto, restaurar es dejar de taparlo — no hay valor previo que guardar ni que se pueda desincronizar. **Se dispara al abrir la lista, no al recibir el foco**: llegar con `Tab` no vacía nada, o tabular por un formulario iría vaciando y restaurando campos a su paso, y para un lector de pantalla el valor cambiaría sólo por pasar por encima.
- **Se mide al abrir y no se vuelve a medir.** Como abrir por click o por flecha parte de la lista completa, la primera medición es contra el panel más alto que va a haber y encogerse después siempre cabe. Remedir en cada tecla traería el volteo intermitente que §11.3 ya documenta para el picker de fechas, y a cambio de un caso —borrar letras hasta que la lista vuelva a crecer— que el `max-height` del panel ya acota.
- **La región viva anuncia cuántas sugerencias hay y existe desde el primer render**, por la misma razón que el aviso de rechazo del `FilePicker`: insertar la región junto con su texto es la forma de que ningún lector de pantalla la lea.
- **`autocomplete` vale `"off"` por defecto**, y es el único control de la base donde no es la cadena vacía: el autorrelleno del navegador dibuja su propio desplegable encima del nuestro. Se puede poner el que haga falta, sabiendo que se acepta ese solape.
...
**DateRangePicker — los rangos que ofrece los declara quien lo usa, no el design system.** `presets` es una lista de `DateRangePreset` (`id`, `label`, y su propio `resolve(hoy)`), no un `type` con ocho nombres cerrados. Es la misma forma que `SelectOption`, y lo que deja que «Ejercicio fiscal» —que empieza en abril y no es asunto de `shared/ui/`— viva en la pantalla que lo necesita. `RANGOS_HABITUALES` son los ocho de fábrica, y son sólo el valor por defecto: se pueden reordenar, recortar o ignorar. Con `presets: []` queda sólo el rango escrito. Para que escribir uno propio no sea reimplementar fechas, el módulo exporta la aritmética (`sumarDias`, `sumarMeses`, `inicioDeSemana`, `inicioDeMes`, `finDeMes`, `inicioDeAno`…). El catálogo se declara como **constante fuera de la clase**, igual que `ROLES` en `contact-role.ts`: uno construido inline en la plantilla sería un array nuevo en cada detección de cambios.

- **Las fechas son texto `YYYY-MM-DD`, nunca un `Date`**, y la aritmética va en UTC aunque la fecha sea civil. `new Date("2026-09-09")` es medianoche **UTC**, que en medio mundo se lee como el día anterior; y sumar 86 400 000 ms en hora local se salta el día del cambio de hora. Lo enuncian los tests «no se salta el día del cambio de hora» y «no corre el día por la zona horaria» de [date-range.spec.ts](src/app/shared/ui/date-range-picker/date-range.spec.ts). `hoyISO()` es la excepción: se construye con los componentes **locales**, porque «hoy» es el de quien mira la pantalla.
- **Un preset aplica y cierra al instante; el rango escrito espera a *Aplicar*.** Un click es una decisión completa. Dos campos a medio rellenar no lo son: aplicar mientras se teclea dejaría el modelo con `from` puesto y `to` vacío. El panel edita un borrador que se siembra al abrir, así que **cerrar sin aplicar no toca el valor**.
- **Los periodos en curso terminan hoy, no al final del periodo.** «Este mes» va del día 1 a hoy; «el mes pasado», que ya terminó, sí está completo. Quien necesite el mes natural entero lo declara con `inicioDeMes`/`finDeMes` — que es exactamente para lo que el catálogo está abierto.
- **El valor guarda las dos fechas y, si vino de un preset, su `presetId`.** Las fechas son la verdad; el id sólo decide si el botón dice «Este mes» o «1 – 30 sept 2026», y desaparece al escribir un rango a mano. Un id que ya no esté en `presets` cae al rango formateado en vez de romper. Al backend se le mandan `from` y `to`.
- **`min` y `max` se llaman `minDate` y `maxDate`.** Los dos primeros están **reservados por `FormValueControl`**, donde son del tipo del valor —un `DateRange`, no una fecha suelta—, y usarlos no compila. Se atan además al otro campo (el `max` del «desde» es el «hasta», y al revés), que es lo que impide el estado imposible sin inventar reglas; aun así se valida al aplicar, porque el nativo no impide teclear. **Un preset que se sale de los límites se deshabilita, no se recorta**: «Este año» recortado a cinco semanas seguiría diciendo «Este año». Los presets se resuelven al abrir el panel, con el `hoy` de ese momento, y `elegir()` los vuelve a comprobar por si el DOM no lo hizo.
- **El `Esc` lo pone el componente, no la pila de `KeyboardService` (§11.6)**, y por la razón que §11.3 ya anticipó para el `ConfirmDialog`: la pila ignora a propósito lo que nace dentro de un campo, y aquí el foco suele estar justo dentro de un `<input type="date">`. Es también lo que mantiene `shared/ui/` sin importar nada de `core/`. Pulsar fuera (`pointerdown` en captura) y sacar el foco del componente con `Tab` cierran igual; el foco vuelve al disparador sólo cuando se cierra a propósito.
- **El panel se ancla con `position: absolute`, no `fixed`**, para que se desplace con el contenido: con `fixed` habría que recolocarlo en cada scroll o cerrarlo. A cambio hay que medir, porque `.shell__content` tiene `overflow-y: auto` y eso computa el eje X a `auto` también: un panel que se salga **no se recorta, genera barra de scroll** — el mismo fallo que el `.sr-only` del punto de abajo. La zona contra la que se mide es la del **primer ancestro que hace scroll** —`.shell__content`, que empieza debajo de la barra—, recortada al viewport; medir contra el viewport dejaba que un panel volteado se metiera debajo de la barra, donde el scroll no llega. Al abrir se enfoca con `preventScroll` y se mide después: el `focus()` a secas desplazaba la página para que cupiera el panel debajo, la medición lo volteaba hacia arriba por medio píxel, y el usuario veía las dos cosas —un salto de 237 px y el panel encima—. Ahora la página no se mueve, el panel se voltea si cabe encima, y sólo cuando no cabe en ningún lado se trae a la vista con `scrollIntoView`. **El volteo se reinicia en cada apertura**: medir con el volteo anterior puesto invertía la decisión una vez sí y otra no, y lo enuncia el test «vuelve a medir desde abajo en cada apertura».
- **El calendario es el nativo del motor.** `color-scheme` (§11.2) es lo que lo hace salir oscuro, y no se le oculta el icono para no depender de `showPicker()`. El del disparador sí es nuestro. **Hay que ejercerlo en la ventana de Tauri**, no sólo en `pnpm start`: §11.2 ya documenta un caso —el popup del `<select>`— donde el motor cumple a medias.

**DateTimeRangePicker — el mismo patrón con reloj, y un valor que no es el mismo tipo.** No es el anterior con una perilla `granularity`, y la razón es dura: `FormValueControl<T>` se tipa por `T`, así que un solo control obligaría a todos sus consumidores a desambiguar `DateRange | DateTimeRange` en cada lectura. Con dos tipos, además, pasarle `RANGOS_HABITUALES` al que lleva hora **no compila** — su `resolve` recibe un instante, no una fecha. La separación de §11.9 hecha tipo.

- **El catálogo por defecto es otro, y ahí está la gracia.** `RANGOS_HABITUALES_CON_HORA` estrena «Últimos 15 min», «Última hora» y «Últimas 24 h», que no existen sin reloj y que son la razón de querer este componente. Son duración **exacta**, medida sobre instantes. Los de periodo en curso extienden la regla de arriba: terminan **ahora**, no al final del día.
- **`minuteStep` va en minutos; el `step` del nativo, en segundos.** Uno es la perilla y el otro la traducción (`minuteStep * 60`). Los minutos que no encajan **se rechazan al aplicar**, porque el widget deja teclear cualquier cosa: el atributo sólo gobierna lo que ofrecen las flechas.
- **`minDateTime` y `maxDateTime` se llaman así por lo mismo que `minDate`/`maxDate`** —`min` y `max` los reserva `FormValueControl`—, son instantes y son **inclusivos**; lo enuncia el test «los límites son inclusivos», que se escribió porque el primer intento confundió el límite con pasarse de él.
- **El rango es semiabierto** (§11.9), así que un `hasta` igual al `desde` se rechaza: en `[desde, hasta)` no significa «ese minuto» sino «nada». `minDateTime` y `maxDateTime` siguen siendo cotas inclusivas — son límites de lo elegible, no los extremos del rango.
- **Una hora local que no existe no se rechaza ni se aplica en silencio.** La que salta el cambio a horario de verano se repinta en el campo con la hora real, el panel dice por qué, y aplicar otra vez la acepta. Rechazarla deja al usuario sin salida; aplicarla callando guarda un momento que no llegó a ver. Cuesta un click más dos días al año.
- **El widget nativo sigue el locale del sistema y la etiqueta no.** El campo puede salir en 12 h con su segmento AM/PM mientras `formatearRangoConHora` usa `Intl` con `"es"` y sale en 24 h. Sólo se nota en un equipo con locale de 12 h, y la única forma de alinearlos sería dejar de usar el nativo. No compensa. El calendario y el reloj hay que ejercerlos **en la ventana de Tauri**, con la misma advertencia que el picker de fechas.
...
- **Bloquea la ventana entera, barra de título incluida.** `showModal()` deja inerte todo el webview menos el diálogo, y con `decorations: false` (§3.7) la barra *es* webview: mientras hay un modal abierto no responden el tema, la campana, los botones de ventana ni el arrastre. Medido: un click sobre el botón de tema no hace nada y `elementFromPoint` sobre la barra devuelve el `<dialog>`. Sigue vivo lo que no pasa por el webview —Alt+Space, Alt+F4, Win+flechas, la barra de tareas—, así que la ventana no queda secuestrada. Por eso el modal es **para decidir y nada más**: nada que tarde va detrás de uno, y editar un registro se hace en el panel de la propia pantalla (§3.11).
- **El velo no se recorta para dejar ver la barra.** Se intentó al revés y es mentira: si la barra está inerte, tiene que verse inerte. Recortar el `::backdrop` pinta menos velo y no devuelve ni un click.
- **Pulsar el fondo no cierra.** Perder por un click perdido lo que ibas a confirmar es el peor final posible. Se sale por Cancelar o por Esc, los dos deliberados.
- **El Esc lo pone el componente.** El `cancel` nativo se atiende y se cancela con `preventDefault()`, para que el diálogo no pueda cerrarse a espaldas del signal que lo gobierna: cerrar siempre lo decide quien lo abrió. Y un `keydown` propio cubre el mismo gesto donde el motor no manda ese aviso, que es justo el camino del punto siguiente.
- **Sin `showModal()` el aviso sale igual, sin velo y sin bloquear.** Si el motor no lo trae, el componente cae al atributo `open`: el diálogo se dibuja en el flujo y sus dos botones funcionan. Peor sería un botón que no hace nada — el mismo criterio con el que §3.10 eligió enseñar un aviso mal rotulado antes que ninguno.
- **`busy` es la promesa de «no toques nada hasta que termine»**: deshabilita cancelar, ignora el Esc y deja el spinner en confirmar. Es la única razón por la que el diálogo puede seguir abierto después de confirmar.
- **En la variante destructiva el foco arranca en Cancelar**, en la normal en Confirmar: con `Enter` no se borra nada sin querer.
- **Lleva `margin: auto` explícito.** El `* { margin: 0 }` del reset (§11.1) le quita al `<dialog>` el centrado que le da el navegador y lo deja pegado a la esquina superior izquierda.
- **El velo es `--backdrop` con un literal como fallback del `var()`.** `::backdrop` no hereda las custom properties en los motores anteriores a Chrome 122 / Safari 17.4, y ahí la variable se quedaría sin valor: sin el fallback el velo sale transparente. Es la única excepción viva a la regla de §11.1, y va escrita en esa línea.
- **El fondo sigue desplazándose con la rueda** (medido: 200 → 295 px). No se bloquea: lo que importa —clicks, foco y teclado— ya está bloqueado, y bloquearlo costaría una regla global metiéndose en las clases del shell más un salto de 15 px al abrir cada diálogo. Si algún día estorba, la salida es un signal que lea el shell, no CSS global.

Los controles de formulario gatean el mensaje de error tras `touched`: enseñar "requerido" en un formulario recién abierto es hostil.

**Los seis controles implementan `focus()` y asocian su mensaje.** El `focus()` no es opcional: el contrato de Signal Forms dice que, si falta, intentará enfocar el elemento anfitrión, y los anfitriones aquí son `display: block` sin `tabindex`, así que enfocar el primer campo inválido desde la API fallaba en silencio. El mensaje —error o ayuda— lleva el id `<control>-msg` y el nativo lo apunta con `aria-describedby`; sin eso el lector de pantalla decía «inválido» sin decir por qué, y el `hint` no se leía nunca.
...
- **La pila se calla mientras haya un `<dialog open>`.** Un modal es modal para todo. El `keydown` sigue llegando a `window` aunque el resto esté inerte (medido), así que sin esa guarda un Esc cerraría el diálogo **y** saldría de pantalla completa en la misma pulsación. Por eso el Esc del `ConfirmDialog` (§11.3) es suyo y no un registro más de la pila: además, la pila ignora a propósito lo que se teclea dentro de un campo, y un diálogo que algún día lleve uno no podría cerrarse desde él.
...
Registrado hoy en el shell: **Ctrl/Cmd + Alt + T** (tema claro/oscuro), **F11** (pantalla completa, §3.7) y dos **Esc** que se apilan y sólo existen mientras hacen falta — cerrar el panel de notificaciones (§3.10) y salir de la pantalla completa, en ese orden de prioridad cuando los dos están activos. Los de la styleguide se registran en su propio componente y desaparecen al salir de la página — el banco de pruebas de `/styleguide` enseña justamente eso.
...
**La conversión entre UTC y la hora del equipo vive en dos funciones, y la validación va sobre el instante.** Son `aLocal` y `aInstante` de [date-time-range.ts](src/app/shared/ui/date-time-range-picker/date-time-range.ts), el único sitio del sistema que mira la zona del equipo; a su lado conviven dos familias que no se mezclan — la aritmética de instantes (`sumarMinutos`, `sumarHoras`), exacta y sin zona, y las fronteras civiles (`inicioDeDiaLocal`, `sumarDiasLocal`, `inicioDeMesLocal`…), que devuelven instantes pero se calculan donde estás. Comparar dos horas locales como texto es lo que funciona con fechas civiles y es una trampa con horas: el mapa de hora local a instante no conserva el orden estricto al cruzar un cambio de offset, así que se convierte primero y se compara después. Dos hechos del motor que hay que anclar en tests y no descubrir en producción: una hora local **inexistente** —la que salta el cambio a horario de verano— la empuja hacia adelante (`2026-03-29T02:30` en Madrid vuelve como 03:30), y una hora **ambigua** —la que ocurre dos veces al volver— se resuelve por la primera ocurrencia. Los tests que ejerzan esto tienen que fijar `process.env.TZ`: la zona de desarrollo puede no tener horario de verano, y entonces no prueban nada.

**Un rango de instantes es semiabierto; uno de fechas civiles, inclusivo; y `comoVentana` es la traducción.** «Del 1 al 30 de septiembre» incluye el 30, que es como se lee un rango de calendario. Un rango de instantes no puede cerrarse igual: con los segundos siempre en cero, una cota final inclusiva a las 23:59 **pierde los últimos 59 segundos del día**, y con `created_at` en milisegundos eso es un registro que existe y no sale en «ayer». Así que `[desde, hasta)`: la cota final es la medianoche del día siguiente y el filtro es `desde <= x < hasta`. De ahí que el módulo no exporte ningún `finDe*` — en un intervalo semiabierto el final de un día **es** el principio del siguiente, y componer `inicioDeDiaLocal` con `sumarDiasLocal` lo dice sin inventar un nombre que engañe. Un rango de anchura cero deja de ser válido, porque ya no significa «ese minuto» sino «nada».

`comoVentana(rango)` es el único puente entre los dos mundos: recibe el `DateSpan` civil del `DateRangePicker` y devuelve la ventana de instantes en la zona del equipo, o `null` si el rango civil no vale. **Lo llama quien filtra, no el componente**: meter la zona dentro del valor de la fecha —o hacer que el picker devuelva también la ventana— obligaría a una fecha de nacimiento a cargar una zona que no tiene, que es justo la distinción que esta sección existe para sostener. En `America/Mexico_City`, «ayer» pasa de `{from: "2026-09-08", to: "2026-09-08"}` a `2026-09-08T06:00:00Z – 2026-09-09T06:00:00Z`; leído como UTC sin más, habría empezado a las 18:00 del día anterior. Se ve en vivo en `/styleguide`, bajo el rango de fechas.
...
**Con temporizadores falsos el orden importa, y no es siempre el mismo.** `whenStable()` usa `setTimeout`, así que con el tiempo ya congelado no resuelve: por eso [gesture-button.spec.ts](src/app/shared/ui/gesture-button/gesture-button.spec.ts) monta primero y congela después — sus temporizadores nacen del `pointerdown`. El `Toast` obliga a lo contrario, porque el suyo arranca al montarse: congela **antes** y renderiza con `fixture.detectChanges()`, que es síncrono y ejecuta los `effect()` del componente. La regla: si el componente arranca temporizadores en el montaje, `whenStable()` no sirve.

**Fijar la zona horaria en un spec tiene dos trampas, y las dos envenenan la suite en silencio.** `process.env["TZ"]` sí surte efecto en caliente en Node 24, así que basta un `beforeAll`; lo que no funciona es deshacerlo. `delete process.env["TZ"]` **no** devuelve la zona del sistema —se queda la última fijada—, y guardar el valor anterior tampoco sirve cuando la variable no estaba definida, que es el caso normal: reasignarlo escribe la cadena `"undefined"` y la zona pasa a `Etc/Unknown`. Lo que hay que guardar es la zona **resuelta** (`Intl.DateTimeFormat().resolvedOptions().timeZone`) y volver a ponerla por nombre; el `afterAll` de [date-time-range.spec.ts](src/app/shared/ui/date-time-range-picker/date-time-range.spec.ts) lo comprueba con un `expect`. Sin eso, cualquier spec que corra después en el mismo worker mide en otra zona sin que nada avise.

Y por lo mismo, **un `Intl.DateTimeFormat` guardado en una constante de módulo congela la zona del momento en que se importó**. `date-range.ts` puede cachearlos porque los suyos llevan `timeZone: "UTC"`; los de `date-time-range.ts` se construyen en cada llamada, que es lo que deja fijar la zona desde el test.
...
**jsdom no implementa `showModal()`.** El `ConfirmDialog` cae al atributo `open` cuando falta (§11.3), así que sus tests ejercen el camino degradado y con eso cubren la lógica: qué se emite, qué no se puede cancelar y dónde arranca el foco. El modal de verdad —capa superior, velo, foco atrapado, barra inerte— sólo se comprueba en un navegador.
...
**Tampoco mide.** `getBoundingClientRect` devuelve ceros, `clientHeight` y `clientWidth` valen 0 y `scrollIntoView` no existe. Un test de colocación simula la geometría sobre `Element.prototype` —con `this` para distinguir el disparador del panel— y define las dos medidas del `documentElement` como propiedades propias; el patrón está en [date-range-picker.spec.ts](src/app/shared/ui/date-range-picker/date-range-picker.spec.ts), que desde la extracción de `AnchoredPanel` es también donde se prueba la colocación de los dos pickers. Fuera de ese bloque, el componente ve altura 0 y no coloca nada, que es lo que deja al resto del spec ignorar la geometría.
...
pnpm test
cd src-tauri && cargo test
cd src-tauri && cargo test --lib -- --ignored   # el aviso del SO, que sale en pantalla (§12)
